  ...
  const Permission = async () => {
    try {
      const granted = await PermissionsAndroid.request(
        ...
        {
          title: 'Cool Photo App Loction Permission',
          message:
            'Cool Loction App needs access to your Loction ',
      ...
      <View style={tw`p-5 flex justify-center gap-3  bg-[#133fdd] bg-emerald-700 shadow-lg  rounded-3xl  w-[100%] `}>
    
        {
          wDataForcast.map((e,i)=>{
            return <Card key={i} temp={e.main.temp} weatherStatus={e.weather.description} date={e.dt_txt} />
          })
        }
      </View>
